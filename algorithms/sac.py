...
def apply_squashing_func(sample, logp):
    ...
    squashed_action_logp = logp - tf.reduce_sum(tf.log(1 - squashed_action ** 2 + 1e-6), axis=1)  # incurred by change of variable
...
class QNetwork(tf.keras.layers.Layer):
    ...
    def __init__(self, hidden_dim=64, num_critics=2):
...
class SAC(tf.keras.layers.Layer):
    ...
    def __init__(self, env, ent_coef='auto', seed=0, hidden_dim=64):
